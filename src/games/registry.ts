 ...
 * ゲームによっては問題データ(*Data.ts)が数百KB規模になるものがあり
 * (例: 倉頡パズル・メイク10)、すべて起動時の1つのJSファイルにまとめてしまうと
 * アプリ起動時にまったく使わないゲームのデータまで読み込むことになってしまう。
 * 実際にそのゲームを選んだときだけ、該当するコードとデータを読み込むことで、
 * 初回表示を軽くする。`PlusChallenge.tsx`側で`<Suspense>`により
 * 読み込み中の表示を出している。
 */
