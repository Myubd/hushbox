...
また、ダウンロードするモデルファイル/トークナイザの真正性については、アプリのビルド時にSHA-256を埋め込んで検証する仕組み(`ModelSpec::expected_sha256` / `tokenizer_expected_sha256`)を用意しています。実際の値は`scripts/fetch_model_pins.py`で取得できますが、このリポジトリのビルド・レビュー環境からはHugging Faceへ到達できなかったため、現時点では値が未設定(`None`)のままです。ネットワークに到達できる開発者が同スクリプトを実行し、`llm_engine.rs`の`available_models()`内`PIN_TODO`コメント箇所を実際の値で埋めることを推奨します。
...
- PII検出の網羅性: `pii_benchmark.rs`でラベル付きコーパス(約20文)に対するrecall/false positive rateを計測するベンチマークテストを追加(`cargo test pii_benchmark -- --nocapture`で数値を確認可能)。設計方針通り、recallは高い閾値(95%以上)で維持されているかを毎回検証
- プロンプト生成(`prompts.rs`): ユニットテストで検証済み
- モデルロード周りの安全性(`llm_engine.rs`): キャッシュ検証・モデル切替時のatomicなスワップ・同時ロード時の排他制御・会話履歴のトークン予算管理・アプリ埋め込みSHA-256 pinとの不一致検出を、計24件のユニットテストで検証済み(実モデル不要、ネットワーク不要で実行可能)
- 安全ポリシーエンジン(`safety_policy.rs`, 新規): 自傷・他害・性的内容・危険物等をLLM呼び出し前にRust側で決定的に判定し、Dangerousの場合はLLMを呼ばず固定の安全な応答(相談窓口の案内を含む)を返す。system promptのルールだけに頼らない多層防御
- Tutor State Machine(`tutor_state.rs`, 新規): 「ヒントを段階的に出す」対話の進行(理解確認→ヒント1〜3→種明かし→理解度確認)をLLMの都度の判断に委ねず、Rust側の状態として管理。各段階でLLMに許可する範囲(答えを言って良いか等)を明示的に縛る。※ 現時点ではバックエンドAPI(`start_tutor_session`/`advance_tutor_session`コマンド)のみで、チャットUIへの配線は未実装
