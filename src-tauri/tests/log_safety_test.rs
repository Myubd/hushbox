//! ログに生の会話内容・PIIを出力していないことを検証する統合テスト。
//! `network_boundary_test.rs`と同じ発想(通信境界ではなくログ出力に適用)。
//!
//! `src/`以下のログマクロ呼び出しをスキャンし、フォーマット文字列内の
//! `{変数名}`インライン補間が、会話内容を保持していそうな変数名
//! (`message`/`content`/`text`等)を含んでいないか確認する。
//!
//! 制限: フォーマット文字列外の位置引数までは解析しない。変数名ベースの
//! ヒューリスティックなので万能ではない。該当行に`// log-safety: allow`
//! コメントがあれば例外として許可する(理由を併記すること)。
...
fn no_raw_conversation_content_in_log_macros() {
    ...
    for path in &files {
        ...
        for (line_no, line) in content.lines().enumerate() {
            ...
            // フォーマット文字列(最初の"..."リテラル)のみ対象。複数行にまたがる呼び出しは対象外。
